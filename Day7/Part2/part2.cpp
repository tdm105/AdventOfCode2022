...
int findSmallestDeletedDir(const Directory& dir, int spaceneeded)
{
    int size = 0;
    ...
    {
        int oldsize = size;
        size = findSmallestDeletedDir(x,spaceneeded);
        if(size - spaceneeded > 0)
        {
            if(size - spaceneeded >= oldsize - spaceneeded && oldsize - spaceneeded > 0)
            {
                size = oldsize;
            }
        }
        ...
        {
            if(oldsize - spaceneeded > 0)
            {
                size = oldsize;
            }
        }
    }
    int dirdiff = dir.size - spaceneeded;
    int sizediff = size - spaceneeded;
    if(dirdiff > 0)
    {
        if(sizediff > 0)
        {
...
int main(int argc, char const *argv[])
{
    ...
    else
    {
        ...
        int spaceneeded = unusedneeded - unusedspace;
        int filesize = findSmallestDeletedDir(root,spaceneeded);
        std::cout << filesize << "\n";
    }
    return 0;
}